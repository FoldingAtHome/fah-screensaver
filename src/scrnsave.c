...
static VERIFYPWDPROC VerifyScreenSavePwd = 0;
...
int APIENTRY WinMain(HINSTANCE hInst, HINSTANCE hPrevInst, LPSTR CmdLine,
                     int nCmdShow) {
  LPSTR p;
  OSVERSIONINFO vi;
  ...
  if (vi.dwPlatformId == VER_PLATFORM_WIN32_WINDOWS) {
    ...
    if (RegOpenKey(HKEY_CURRENT_USER, REGSTR_PATH_SCREENSAVE, &hKey) ==
      ...
      res = RegQueryValueEx(hKey, REGSTR_VALUE_USESCRPASSWORD,
                            0, &type, (PBYTE) &check_pwd, &size);

      if (check_pwd && res == ERROR_SUCCESS) {
        hPwdLib = LoadLibrary(TEXT("PASSWORD.CPL"));
        if (hPwdLib)
          VerifyScreenSavePwd =
            (VERIFYPWDPROC)GetProcAddress(hPwdLib, szVerifyPassword);
      }
  ...
  for (p = CmdLine; *p; p++) {
    switch (*p) {
    case 'S':
    case 's': return LaunchScreenSaver(0); // start screen saver
...
static int LaunchScreenSaver(HWND hParent) {
  ...
  // register classes, both user defined & classes used by screen saver library
  if (!RegisterClasses()) {
    MessageBox(0, TEXT("RegisterClasses() failed"), 0, MB_ICONHAND);
    goto restore;
  }
  ...
  hMainWindow = CreateWindowEx(hParent ? 0 : WS_EX_TOPMOST, CLASS_SCRNSAVE,
                               TEXT("SCREENSAVER"), style, rc.left, rc.top,
                               rc.right, rc.bottom, hParent, 0,
                               hMainInstance, 0);
...
LRESULT WINAPI DefScreenSaverProc(HWND hWnd, UINT msg, WPARAM wParam,
                                  LPARAM lParam) {
...
void WINAPI ScreenSaverChangePassword(HWND hParent) {
  ...
  if (hMpr) {
    ...
    if (ChangePassword) ChangePassword(TEXT("SCRSAVE"), hParent, 0, 0);
